...
module.exports = {
  ...
  methods: {
    createPendingMeal: function(mealId, item) {
      ...
      this.$store.commit(
        "addOrderToPendingMealOrders",
        this.getCurrentMealOrder
      );
      ...
      setTimeout(function() {
        if (self.$store.getters.currentMealOrder.isDeleted == false) {
          ...
          self.$store.commit(
            "removeOrderFromPendingMealOrders",
            self.getPendingMealOrders.findIndex(o => o.id === self.counter)
          );
        }
      }, 5000);
